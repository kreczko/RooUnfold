...
public:
	...
	RooUnfoldResponse(Int_t nm, Double_t mlo, Double_t mhi, Int_t nt, Double_t tlo, Double_t thi, const char* name = 0,
			const char* title = 0);
	// constructor - measured and truth only used for shape
	RooUnfoldResponse(const TH1* measured, const TH1* truth, const char* name = 0, const char* title = 0);
	// create from already-filled histograms
	RooUnfoldResponse(const TH1* measured, const TH1* truth, const TH2* response, const char* name = 0,
			const char* title = 0);
	// create from already-filled histograms
	RooUnfoldResponse(const TH1* measured, const TH1* truth, const TH1* fakes, const TH2* response, const char* name = 0,
				const char* title = 0);
	...
	virtual RooUnfoldResponse& Setup(const TH1* measured, const TH1* truth, const TH1* fakes, const TH2* response);
...
inline RooUnfoldResponse::RooUnfoldResponse() :
		TNamed() {
	// RooUnfoldResponse default constructor. Use Setup() to set values.
	Init();
}

inline RooUnfoldResponse::RooUnfoldResponse(const char* name, const char* title) :
		TNamed(name, title) {
	// RooUnfoldResponse default named constructor. Use Setup() to set values.
	Init();
}

inline RooUnfoldResponse::RooUnfoldResponse(const TString& name, const TString& title) :
		TNamed(name, title) {
	// RooUnfoldResponse default named constructor. Use Setup() to set values.
	Init();
}
...
inline RooUnfoldResponse& RooUnfoldResponse::Setup(Int_t nb, Double_t xlo, Double_t xhi) {
	// constructor -  simple 1D case with same binning, measured vs truth
	return Setup(nb, xlo, xhi, nb, xlo, xhi);
}

inline Int_t RooUnfoldResponse::GetDimensionMeasured() const {
	// Dimensionality of the measured distribution (1=1D, 2=2D, 3=3D)
	return _mdim;
}

inline Int_t RooUnfoldResponse::GetDimensionTruth() const {
	// Dimensionality of the truth distribution (1=1D, 2=2D, 3=3D)
	return _tdim;
}

inline Int_t RooUnfoldResponse::GetNbinsMeasured() const {
	// Total number of bins in the measured distribution
	return _nm;
}

inline Int_t RooUnfoldResponse::GetNbinsTruth() const {
	// Total number of bins in the truth distribution
	return _nt;
}

inline const TH1* RooUnfoldResponse::Hmeasured() const {
	// Measured distribution, including fakes
	return _mes;
}

inline TH1* RooUnfoldResponse::Hmeasured() {
	return _mes;
}

inline const TH1* RooUnfoldResponse::Hfakes() const {
	// Fakes distribution
	return _fak;
}

inline TH1* RooUnfoldResponse::Hfakes() {
	return _fak;
}

inline const TH1* RooUnfoldResponse::Htruth() const {
	// Truth distribution, used for normalisation
	return _tru;
}

inline TH1* RooUnfoldResponse::Htruth() {
	return _tru;
}

inline const TH2* RooUnfoldResponse::Hresponse() const {
	// Response matrix as a 2D-histogram: (x,y)=(measured,truth)
	return _res;
}

inline TH2* RooUnfoldResponse::Hresponse() {
	return _res;
}

inline const TVectorD& RooUnfoldResponse::Vmeasured() const {
	// Measured distribution as a TVectorD
	if (!_vMes)
		_cached = (_vMes = H2V(_mes, _nm, _overflow));
	return *_vMes;
}

inline const TVectorD& RooUnfoldResponse::Vfakes() const {
	// Fakes distribution as a TVectorD
	if (!_vFak)
		_cached = (_vFak = H2V(_fak, _nm, _overflow));
	return *_vFak;
}

inline const TVectorD& RooUnfoldResponse::Emeasured() const {
	// Measured distribution errors as a TVectorD
	if (!_eMes)
		_cached = (_eMes = H2VE(_mes, _nm, _overflow));
	return *_eMes;
}

inline const TVectorD& RooUnfoldResponse::Vtruth() const {
	// Truth distribution as a TVectorD
	if (!_vTru)
		_cached = (_vTru = H2V(_tru, _nt, _overflow));
	return *_vTru;
}

inline const TVectorD& RooUnfoldResponse::Etruth() const {
	// Truth distribution errors as a TVectorD
	if (!_eTru)
		_cached = (_eTru = H2VE(_tru, _nt, _overflow));
	return *_eTru;
}

inline const TMatrixD& RooUnfoldResponse::Mresponse() const {
	// Response matrix as a TMatrixD: (row,column)=(measured,truth)
	if (!_mRes)
		_cached = (_mRes = H2M(_res, _nm, _nt, _tru, _overflow));
	return *_mRes;
}

inline const TMatrixD& RooUnfoldResponse::Eresponse() const {
	// Response matrix errors as a TMatrixD: (row,column)=(measured,truth)
	if (!_eRes)
		_cached = (_eRes = H2ME(_res, _nm, _nt, _tru, _overflow));
	return *_eRes;
}

inline Double_t RooUnfoldResponse::operator()(Int_t r, Int_t t) const {
	// Response matrix element (measured,truth)
	return Mresponse()(r, t);
}

inline Int_t RooUnfoldResponse::GetBin(const TH1* h, Int_t i, Bool_t overflow) {
...
inline Double_t RooUnfoldResponse::GetBinContent(const TH1* h, Int_t i, Bool_t overflow) {
	// Bin content by vector index
	return h->GetBinContent(GetBin(h, i, overflow));
}

inline Double_t RooUnfoldResponse::GetBinError(const TH1* h, Int_t i, Bool_t overflow) {
	// Bin error   by vector index
	return h->GetBinError(GetBin(h, i, overflow));
}

inline Int_t RooUnfoldResponse::Miss(Double_t xt) {
	// Fill missed event into 1D Response Matrix
	return Miss1D(xt);
}

inline Int_t RooUnfoldResponse::Miss(Double_t xt, Double_t w) {
	// Fill missed event into 1D (with weight) or 2D Response Matrix
	return _tdim == 2 ? Miss2D(xt, w) : Miss1D(xt, w);
}

inline Int_t RooUnfoldResponse::Miss(Double_t xt, Double_t yt, Double_t w) {
	// Fill missed event into 2D (with weight) or 3D Response Matrix
	return _tdim == 3 ? Miss(xt, yt, w, 1.0) : Miss2D(xt, yt, w);
}

inline Int_t RooUnfoldResponse::Fake(Double_t xr) {
	// Fill fake event into 1D Response Matrix
	return Fake1D(xr);
}

inline Int_t RooUnfoldResponse::Fake(Double_t xr, Double_t w) {
	// Fill fake event into 1D (with weight) or 2D Response Matrix
	return _mdim == 2 ? Fake2D(xr, w) : Fake1D(xr, w);
}

inline Int_t RooUnfoldResponse::Fake(Double_t xr, Double_t yr, Double_t w) {
...
inline Bool_t RooUnfoldResponse::UseOverflowStatus() const {
	// Get UseOverflow setting
	return _overflow;
}

inline Double_t RooUnfoldResponse::FakeEntries() const {
